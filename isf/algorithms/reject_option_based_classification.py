...
try:
    from aif360.algorithms.postprocessing.reject_option_classification import replacement_check
except ImportError:
    print(traceback.format_exc())
    print('\nPlease replace the referenced aif360 reject_option_classification.py as shown in the following command.')
    print('\n$ patch {aif360 installed directory}/algorithms/postprocessing/reject_option_classification.py patches/reject_option_classification.patch\n\nor')
...
class RejectOptionClassification(PostProcessing):
    ...
    def __init__(self, options):
        ...
        self.accuracy_metric_name = options['accuracy_metric_name'] if 'accuracy_metric_name' in options else 'Balanced Accuracy'
